...
# Para a parte 1 rodar os arquivo de configuração do Postgres para a replicação kube exec.bat:

kubectl exec db-main-0 -- /bin/bash /opt/pg_copyfile.sh
# Copia os arquivos de configuração que eu envio prontos e configurados de acordo com a indicação do fabricante.

kubectl exec db-main-0 -- /bin/bash /opt/tabelas.sh
# Cria usuários, uma tabela de teste e entradas para termos um teste completo

kubectl exec db-replica-0 -- /bin/bash /opt/pg_backup.sh
# Executa o comando de backup completo e setagem da replicação no pod replica.